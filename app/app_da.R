...
# load Danish data
wheels_all <- st_read("../data_processing/map_data_da.shp")
...
cat_all <- unique(all_yes$cat_da)
# turn NA into a different value on the list
cat_all[1] <- "Vælg/vis alle"
  ...
  navbarPage("Accessible Cities", id = "nav",
      ...
      fluidPage(
        ...
        fluidRow(
          ...
          column(3,
                 ...
                 textInput("place", label = NULL, placeholder = "Skriv her"),
    ...
    tabPanel("Sammenlign", 
      fluidRow(
        ...
        p("Her kan du udforske vores sammenligningsværktøj, der sammenligner byers tilgængelighed på baggrund af tre mål: gennemsnitlig afstand mellem tilgængelige lokationer, antal tilgængelige lokationer pr. km² og andelen af tilgængelige lokationer.", style = "text-align:center;"),
      ...
      fluidRow(
        ...
        column(4,
               p("Andel af tilgængelige steder", style = "text-align:center;"),
               # pie chart icon
        ...
        # column with density
        column(4,
               p("Tilgængelige steder pr. km²", style = "text-align:center;"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$cph, {wheels$data <- wheels_all %>% dplyr::filter(city=="København")})
  ...
  output$map <- renderLeaflet({
    # if no category is chosen show all data
      if (input$amenity=="Vælg/vis alle"){wheels_data <- wheels$data} 
    # if a category is chosen, only display accessible locations in that category
      else {
        wheels_data <- wheels$data
        wheels_data_filtered <- dplyr::filter(wheels_data, whelchr=="yes" & cat_da==input$amenity)
    ...
    leaflet() %>%
      ...
      addLegend(colors = unique(wheels_data$color), labels=unique(wheels_data$label_d), title="Kørestolstilgængelighed <br> for lokationen", opacity=1, group="Base")
  })
  ...
  output$density <- renderPlot({
    ...
    if (nrow(plot_data()) > 0) {
      ggplot(data = plot_data(), aes(x = X, y = density)) +
        geom_bar(stat = "identity", fill = "#009CDE") +
        ylab("Tilgængelige steder pr. km²") +
        xlab("Område") +
